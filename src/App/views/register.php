    ...
    <form action="/register" method="POST" class="grid grid-cols-1 gap-6">
        ...
        <label class="block">
            ...
            <select name="country"
                ...
                <option value="Mexico"
                    <?php echo (($oldFormData['country'] ?? '') === 'Mexico') ? 'selected' : ''; ?>>
                    Mexico
                </option>
        ...
        <label class="block">
            ...
            <input name="socialMediaURL" value="<?php echo  e($oldFormData['socialMediaURL'] ?? ''); ?>"
                type="text"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                placeholder="" />
        ...
        <label class="block">
            ...
            <input name="confirmedPassword"
                ...
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                placeholder="" />
            <?php if (array_key_exists('confirmedPassword', $errors)) : ?>
                <div class="bg-gray-100 mt-2 p-2 text-red-500">
                    <?php echo e($errors['confirmedPassword'][0]); ?>
                </div>
            <?php endif; ?>
        </label>
        <!-- Terms of Service -->
